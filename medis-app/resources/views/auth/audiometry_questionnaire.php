...
<head>
    ...
    <title>Audiometry - Questionnaire</title>
</head>
<body>
<?php
...
$questionnaireRows = $questionnaireRows ?? collect();
$questionnaireTotal = $questionnaireTotal ?? (is_countable($questionnaireRows) ? count($questionnaireRows) : 0);
    ...
    <section class="content">
        <div class="head">
            <div>
                ...
                <p>Review audiometry record list for the selected employee before examination.</p>
        ...
        <div class="toolbar">
            <div class="toolbar-left">
                <input id="questionnaireSearch" type="text" placeholder="Search audiometry record">
            </div>
        ...
        <table class="table">
            ...
            <tbody>
                <?php if ($questionnaireTotal > 0): ?>
                    <?php foreach ($questionnaireRows as $row): ?>
                        <?php
                        ...
                        $editUrl = function_exists('route')
                            ...
                            : 'edit_questionnaire.php?id=' . urlencode((string) $row->audiometry_id);
                        $deleteUrl = function_exists('route')
                            ? route('audiometry.record.delete', ['id' => $row->audiometry_id])
                            : 'delete_questionnaire.php?id=' . urlencode((string) $row->audiometry_id);
                        ...
                        <tr data-status="<?php echo $esc($row->status ?? 'pending'); ?>">
                            ...
                            <td>
                                <a class="table-name-link" href="<?php echo $esc($editUrl); ?>">
                                    <?php echo $esc($name !== '' ? $name : 'Not set'); ?>
                                </a>
                            </td>
        ...
        <div class="bottom">
            <span class="pager">
                <?php echo $questionnaireTotal > 0 ? 'Showing 1-' . number_format($questionnaireTotal) . ' of ' . number_format($questionnaireTotal) . ' records' : 'Showing 0 of 0 records'; ?>
            </span>
            <div>
                <a class="btn" href="<?php echo $esc($backUrl); ?>">Back</a>
                <a class="next" href="<?php echo $esc($nextUrl); ?>">Next</a>
            </div>
        </div>
    </section>
</div>
...
<script>
(function () {
    var searchInput = document.getElementById('questionnaireSearch');
